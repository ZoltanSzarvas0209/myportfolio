...
class OrderModelTest(TestCase):
    ...
    def test_update_total(self):
        ...
        service = Service.objects.create(name="Test Service", price=Decimal("50.00"))
        line_item = OrderLineItem.objects.create(order=self.order, service=service, quantity=2)
...
class OrderLineItemModelTest(TestCase):
    def setUp(self):
        ...
        self.service = Service.objects.create(name="Test Service", price=Decimal("25.00"))

    def test_lineitem_total_calculated(self):
        """Test that lineitem_total is correctly calculated on save"""
        line_item = OrderLineItem.objects.create(order=self.order, service=self.service, quantity=3)
    ...
    def test_order_total_updates_on_lineitem_save(self):
        ...
        line_item = OrderLineItem.objects.create(order=self.order, service=self.service, quantity=2)
